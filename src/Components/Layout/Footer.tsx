...
const Footer = () => {
  ...
  const _pages: number = data.informations ? Number(data.informations.next.split('=')[1]) : 0
  console.log('split', data.informations ? data.informations.next.split('=')[1] : '')
  return (
    <section className='container-fluid'>
      <div className='d-grid gap-2 d-md-flex justify-content-center '>
        <h6 className='text-white'>Cantidad de Registro: {data.informations.count}</h6>
        <h6 className='text-white'>Cantidad de Paginas: {_pages} de {data.informations.pages}</h6>
      </div>
